...
double LennardJonesSystem::calculate_energy() {
	double energy(0.0);
	for (size_t i = 0; i < particles.size()-1; ++i) {
		for (size_t j = i+1; j < particles.size(); ++j) {
			double r = calculate_distance(particles[i], particles[j]);
			energy += calculate_lj_interaction(r);
		}
	}
	return energy;
}
